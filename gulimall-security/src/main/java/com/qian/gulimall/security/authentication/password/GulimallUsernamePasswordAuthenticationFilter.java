 ...
 * GulimallUsernamePasswordAuthenticationFilter is 自定义UsernamePasswordAuthenticationFilter
 * post提交数据
 *
 * @author QIAN
 * Date 2020/04/18
 * Time 17:32
 */
public class GulimallUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public GulimallUsernamePasswordAuthenticationFilter(String loginProcessUrlPassword) {
        super(new AntPathRequestMatcher(loginProcessUrlPassword, "POST"));
    }
...
//            request.removeAttribute(SecurityConstants.DEFAULT_LOGIN_INFO);
            if (loginInfoVo == null) {
                loginInfoVo = RequestHandleUtils.convertToLoginInfoVo(request);
            }
            if (loginInfoVo != null) {
